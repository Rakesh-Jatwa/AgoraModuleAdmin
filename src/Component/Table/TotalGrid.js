...
class TotalGrid extends Component {
    state = {
      width_1 : '50%',
      width_2:'100%',
      wdith_3 : 0
    ...
    build_tobody = (details) =>{
        ...
        for(let i=0;i<this.props.body_loop; i++){
          if(this.props.body_type[i]=='text'){
            _tmp_div.push(<tr><td style={{textAlign:'right'}}> {(this.props.body_text.hasOwnProperty(i)) ? details.body_text[i] : ''} </td><td  className="td_right"  style={{textAlign:'right'}}> {(this.props.body_value.hasOwnProperty(i)) ? this.props.body_value[i] : ''} </td><td className="td_left"  style={{textAlign:'right'}}></td></tr>)
          }
          ...
          <tr>
            {(this.props.table_header) ? this.props.table_header.map((list, index)=>{
              ...
              if(index==this.props.total){
                return   <td style={{textAlign:'right', width: this.state.width_3  - this.props.adjust}}></td>
              }
