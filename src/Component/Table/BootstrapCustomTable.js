...
import {ddmmyy as InvoiceDate} from '../../Component/Dates/TimeZone'
...
class BootstrapCustomTable extends Component {
    ...
    renderShowsTotal(start, to, total) {
      ...
      return (
        <p style={ { color: 'blue' } }>
          {(to > 0) ? (_details) : 0} record(s) found. { Math.ceil(_total_1) } page(s) found
        </p>
      );
    }
    ...
    sortPOMPODate (a, b, order) {
      ...
      if (order === 'asc') {
        let r = 0;
        indices.find(i => r = ((a.POM_PO_Date) ? a.POM_PO_Date.charCodeAt(i) : 0) - (b.POM_PO_Date) ? b.POM_PO_Date.charCodeAt(i) : 1 );
        return r;
      } else if (order === 'desc') {
        let r = 0;

        indices.find(i => r = ((b.POM_PO_Date && b.POM_PO_Date.toString()) ? b.POM_PO_Date.toString().charCodeAt(i) : 0) - ((a.POM_PO_Date && a.POM_PO_Date.toString()) ? a.POM_PO_Date.toString().charCodeAt(i) :0 ));
        return r;
      }
    }
    ...
    inv_mass_textarea(cell, row, enumObject, rowIndex, details) {
      console.log('inv_mass_textarea', this.props.input_remarks)
      let _temp_details ='';
      ...
      return (
        <div>
          ...
          <input onChange={(this.props.change) ? this.props.getInputs :()=>{} } data-invoiceno={enumObject.IM_INVOICE_INDEX}   data-name={details} data-details={cell}  name={cell+'['+enumObject.POM_PO_Index+']'}  type="text" className="form-control" value={(this.props.input_remarks && this.props.input_remarks[details] && this.props.input_remarks[details].value!='null') ? this.props.input_remarks[details].value : '' }/>
        </div>
      )
    }
    ...
    inv_date(cell, row, enumObject, rowIndex, details) {
      if(row){
    ...
    const options = {
        ...
        sizePerPage: (this.props.hasOwnProperty('total') ? this.props.total : 10),  // which size per page you want to locate as default
        ...
        <BootstrapTable ref={(this.props.ref) ? this.props.ref : ''} data={ (this.props.table_body) ? this.props.table_body : [] } cellEdit={cellEditProp} selectRow={this.props.select ? selectRowProp: false} pagination   options={ options } responsive striped   hover search={(this.props.search!='') ? true : true}>
            {this.props.table_header && this.props.table_header.map((list)=>{
                ...
                if(list.dataFormat=="button"){
                  ...
                  customEditor={{ getElement: createSelectModel }}
                  thStyle={{ whiteSpace: 'normal', width: list.width }}
                  dataFormat={this.inv_date.bind(this, list.id)}
                  dataSort={ true }
