...
class BootstrapCustomTable extends Component {
    ...
    sortDates (a, b, order) {
      console.log('sortDates', a, b, order)
    ...
    divButton(cell, row, enumObject, rowIndex, details) {
      console.log('divButton', row)
      return  <div>{(row!='undefined' && typeof row != 'undefined') ? row :'' }</div>
    }
    ...
    const options = {
      ...
      return (
        <Fragment>
        <BootstrapTable ref={(this.props.ref) ? this.props.ref : ''} data={ (this.props.table_body) ? this.props.table_body : [] }  pagination   options={ options } responsive striped   hover search={(this.props.search!='') ? true : true}>
