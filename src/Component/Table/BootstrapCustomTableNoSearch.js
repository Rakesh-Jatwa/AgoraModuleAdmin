...
class BootstrapCustomTableNoSearch extends Component {
    ...
    cdi_unit_cost(cell, row, enumObject, rowIndex, details) {
      console.log('_temp_details', enumObject.appRemarks)
      let _temp_details ='';
      if(enumObject.hasOwnProperty('appRemarks') && enumObject.appRemarks.length > 0){
        _temp_details = enumObject.appRemarks.map((list_details, index)=>{
    ...
    const options = {
      ...
      return (
        <Fragment>
        <BootstrapTable ref={(this.props.ref) ? this.props.ref : ''} data={ (this.props.table_body) ? this.props.table_body : [] } cellEdit={cellEditProp} selectRow={this.props.select ? selectRowProp: false} pagination   options={ options } responsive striped   hover search={(this.props.search!='') ? false : false}>
            {this.props.table_header && this.props.table_header.map((list)=>{
                ...
                else if(list.dataFormat=="cdi_unit_cost"){
                  return  <TableHeaderColumn
                      tdStyle={ { whiteSpace: 'normal', width: list.width, textAlign: 'right' } }
                      dataField={list.id} isKey={(list.key) ? true : false} dataSort
                      editable={ (list.type) ? true : false } customEditor={(list.type) ? { getElement: createSelectModel } : ''}
                      thStyle={{ whiteSpace: 'normal', width: list.width, textAlign: 'right' }}
                      dataFormat={this.cdi_unit_cost.bind(this, list.id)}>{list.name}
                    </TableHeaderColumn>
                }
  ...
  export default BootstrapCustomTableNoSearch;
