...
class NewCreditNote extends Component {
    constructor(props){
        ...
        this.hide_view = this.hide_view.bind(this)
        this.state = {
            products:[],
            start_data:new Date(),
    ...
    componentDidMount(){
        console.log('debit_note', this.props)
        if(this.props.location && this.props.location.page_name=="dashboard"){
           
            this.setState({
                cndn_details : this.props.location.datas
            })
            this.view_cndn_details(this.props.location.datas)
        }
    }

    componentDidUpdate(){
        console.log('debit_note_!', this.props.location)
    }
    ...
    hide_view =() =>{
        console.log('hide_view', this.props.history, this.props.location)
        this.setState({
            show_result : false,
            view_cndn : false,
        })
        ...
        if(this.props.location && this.props.location.redirect_to_tab){
            this.props.history.push({
                pathname:this.props.location.redirect_to_tab
            })
        }
    }
    ...
    render(){
        ...
        const _table_header = [
            ...
            {name : "Creation Date", id:"DNM_DN_DATE", width:'118px', dataFormat:'date'},
        ...
        return <Fragment>
            ...
            <div className="row mt-2">    
                    <div className='col-12 col-md-12'>   
                        <div className="row">
                            <Field type="text" name="PurchaseRequestListing.strDnNo" component={FromInputsParallel} className="form-control" placeholder="Debit Note No. " label="Debit Note No. : " />
