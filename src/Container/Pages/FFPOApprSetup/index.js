...
class PurchaseApprovalSelect extends React.Component {
    ...
    componentDidMount(){
        ...
        if(this.props.location && this.props.location.datas){
            console.log('componentDidMount', this.props.location.datas)
    ...
    submit = async() => {
        let prNo = { 
            "lblPONo": (this.state.update_details && this.state.update_details.pr_no ) ? this.state.update_details.pr_no : '' ,
        ...
        if(_status){
            ...
            localStorage.removeItem('ff_details')
    ...
    async getApprovalLevelList (e){
        ...
        if( this.state && this.state.update_details && this.state.update_details.pr_no){
    ...
    calculationfnc = (data, start, approvetype) => {
        ...
        let pocost = (this.state.update_details) ? this.state.update_details.pr_cost : 0
       
        data.forEach(e => {
            console.log('pocost', this.state.update_details.pr_cost, e.UM_APP_LIMIT, this.state.update_details.pr_cost <= e.UM_APP_LIMIT)
            switch (approvetype) {
    ...
    render() {
        ...
        let approvalList = (this.state.update_details && this.state.update_details.approval_list) ? this.state.update_details.approval_list.map((fieldValue, index) => <option key={index} value={fieldValue.AGA_GRP_INDEX}>{fieldValue.AGM_GRP_NAME}</option>) :'';
            ...
            <Tabs defaultActiveKey="home" transition={false} id="tabs" activeKey={this.state.active_key} onSelect={k => {
                ...
                <Tab eventKey="contact" title="Free Form Purchase Order">
                  <div>
                    ...
                    <div className="tab-content py-3 px-3 px-sm-0">
                        <h6>Raise Free Form Purchase Order </h6>
                        <p>Please select the approval workflow from the approval list and submit the PO.</p>
                        <div className="d-flex bg-info text-white color-white p-1 mt-2">
                        ...
                        <table className="table table-bordered ">
                            <tbody>
                                <tr>
                                    ...
                                    <td>{(this.state.update_details && this.state.update_details.pr_no) ? this.state.update_details.pr_no : '' }</td>
                                </tr>
