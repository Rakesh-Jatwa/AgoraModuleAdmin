...
import {TodayDateSalash, DateMinusTimeLoginYear} from '../../../../../Component/Dates'
...
import {HandlePayment, HandleStatus,NumberFormateEmpty, HandleCategoryType, HandleDocType} from '../../../../../Actions/Common/Functions'
...
let ConverPRListing = props => {
    ...
    let [psd_received_date, setPsdReceivedDate]= useState(new Date)
    let [confirmation_type, setConfirmationType]= useState('')
    ...
    let  _approver_header = [
        ...
        {name : "Performed By", id:"FA_AO", width:'85px', key:false},
        {name : "User ID", id:"AO_NAME", width:'150px'},
        {name : "Date Time", id:"FA_ACTION_DATE", width:'100px', dataFormat:"date"},
   ...
   let onCancel  = () =>{
    ...
    let Approve = async() => {
            ...
            let _final_datas = {
                ...
                "strAction":"verify",
                "frm":"IPPPSD",
                "PSDReceiveDate": (psd_received_date) ? DateMinusTimeLoginYear(psd_received_date) : new Date(),
    ...
    const _table_data_header  = [
        ...
        {name : "Amount (MYR) (excl. SST)", id:"ID_UNIT_COST", width:'125px',formatter: (cellContent, row) => {
            if(props.details && props.details.IM_CURRENCY_CODE=="MYR"){
                ...
                return <div class="text-right">{(_exchange_rate) ? (row.ID_UNIT_COST * _exchange_rate) : row.ID_UNIT_COST}</div>
            }
        
        }},
        {name : "Amount (MYR) (excl. SST)", id:"ID_UNIT_COST", width:'125px',dataFormat:'price'},
        ...
        {name : "Input Tax Code", id:"inputTax", width:'90px',formatter: (cellContent, row) => {
            return <div>
                <select className="form-control" disabled>
                    <option>{(row.ID_GST_INPUT_TAX_CODE) ? row.ID_GST_INPUT_TAX_CODE +'('+row.ID_GST+' %)' : ''}</option>
                </select>
            </div>
        }},
        {name : "Output Tax Code", id:"ID_GST_OUTPUT_TAX_CODE", width:'90px',formatter: (cellContent, row) => {
            return <div>
                <select className="form-control" disabled>
                    <option>{(row.ID_GST_OUTPUT_TAX_CODE) ? row.ID_GST_OUTPUT_TAX_CODE : 'N/A'}</option>
                </select>
            </div>
        }},
    ...
    if(typeof props.details != 'undefined' && props.details.IM_INVOICE_TYPE=="CN" || props.details.IM_INVOICE_TYPE=="DN"){
    ...
    props.change('Filters.UIStartDate', new Date())
                ...
                <div className="row mt-2">
                    <div className="col-md-12">
                        ...
                        <div className="row" >
                            ...
                            <div className="col"><p>{(props.details) ? HandleStatus(props.details.IM_INVOICE_STATUS) : ''}</p></div>
...
<div className="col-md-12 col-12 col-lg-2"><label>SST Amount :</label></div>
                        ...
                        <div className="row" >
                            ...
                            <div className="col"><p>{(props.details) ? TodayDateSalash(props.details.IM_PRCS_SENT) : ''}</p></div>

                            <div className="col-md-12 col-12 col-lg-2"><label>Payment Amount  : </label></div>
                            <div className="col"><p>{(props.details) ? NumberFormateEmpty(parseFloat(props.details.IM_INVOICE_TOTAL)) : ''}</p></div>
                        </div>
                        ...
                        <Fragment>
                            <div className="row" >
                                ...
                                <div className="col"><p>{(props.details) ? TodayDateSalash(props.details.IM_PRCS_RECV) : ''}</p></div>
                            </div>
                            <div className="row" >
                        ...
                        <div className="row doc_start_date" >
                            {/* <div className="date_main_Details"> */}
                            {(props.frompage=="approve") ? <Field type="text" name="Filters.UIStartDate" selected={(psd_received_date) ? psd_received_date : new Date() } component={FormDatePickerParallel} className="form-control" placeholder="PSD Received Date"  label="PSD Received Date " onChange={handleDate} /> :''}
                            {/* </div> */}
                        ...
                        <div className="row" >
                            ...
                            <div className="col"><address className="e2p_address">{(props.details) ? <Fragment>
                             ...
                             {(props.details.IM_POSTCODE) ? props.details.IM_POSTCODE+'\n' : ''}
                        ...
                        <div className="mt-2 row">
                            ...
                            {(props.frompage=="approve" && (block_process==false)) ? 
                                <div className="col_details">
                                    {(props.from_page!="listing" &&  props.attachments && props.attachments[0]!== 'No Files Attached' && props.attachments.length>=1) ? props.attachments.map((list, index) => {
            ...
            {(props.frompage=="approve") ? 
                <div className="row">
                    <div className="col-12 doc_start_date" style={{padding:'0px'}}>  
                        <Field type="text" name="ApproveDto.strRemark" component={FromTextareaParallel} className="form-control" placeholder=" Remarks" label=" Remarks"  onChange={(e)=>setRemarks(e.target.value)}/>
                        {/* <div className="col-12 col-sm-6">
                        <div className="row mt-2 doc_start_date">
            ...
            <div className="mt-2 mb-5 row">
              {(props.frompage=="approve" && (block_process==false)) ? 
                <Fragment>
                    <div className="col-lg-auto col-md"><button type="button" className="btn btn-outline-success btn-sm" onClick={()=>confirm_function('approve',' accept this Document')}> Accept</button> </div>
                    <div className="col-lg-auto col-md"><button type="button" className="btn btn-outline-secondary btn-sm" onClick={()=>confirm_function('reject',' reject this Document')} >Reject</button></div> 
                </Fragment>
                : ''}
                {(props.frompage!="approve") ? <div className="col-lg-auto col-md"><button type="button" className="btn btn-outline-info btn-sm" onClick={()=>{props.setInvoiceIndex((props.details) ? props.details.IM_INVOICE_INDEX : '', (props.details) ? props.details.IM_INVOICE_NO : '' , (props.details) ? HandleStatus(props.details.IM_INVOICE_STATUS) : '')}}>View Audit</button></div> : ''}
