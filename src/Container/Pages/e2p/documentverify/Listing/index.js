...
import {GetE2PApprovalAprovRej, GetE2PApprovalDetails} from '../../../../../Actions/Approver'
...
class ApprovalListing extends Component{
    constructor(props){
        ...
        this.state = {
            ...
            all_check_value:[],
            loading:false,
            checked_details:[1 ,2,3,19,4,11,12,13,14,15, 17,18],
            check_value:false,
            show_result:false,
            vendor_details :'',
            object_initial : {"docno": "", "doctype": "", "docsdt": '', "docedt": '', "verifiedsdt": '', "verifiededt": '',vendor:'' , "docstatus": "", "frm":"IPPAO"},
            modal_title:'',
            modal_body:'',
            modal:false,
        }
    ...
    static getDerivedStateFromProps (nextProps, prevState){
        if(!prevState.show_result && nextProps.approval_list && nextProps.approval_list && nextProps.approval_list.data){
            ...
            return {
                ...
                attachments : (nextProps.approval_details && nextProps.approval_details.displayAttachFile) ? nextProps.approval_details.displayAttachFile : [],
    ...
    handlefromsubmit(values){
        let  {Filters, all_check_value} = values;
        let _details = values.Filters;
        let _checked_details = [];

        if(Filters && Filters.status && Filters.status.length > 0){
            Filters.status.forEach((list_details, index)=>{
                if(list_details){
        ...
        let _form_value = {}
        let _initial_obj = {}
        _form_value = Object.assign({}, _initial_obj,(Filters) ? Filters : _form_value )
        console.log('_form_value', _form_value, Filters)
        
        _form_value.docstatus = (_checked_details && _checked_details.length > 0) ? _checked_details.toString() : '';
        _form_value.docsdt = (_form_value && _form_value.docsdt) ? FromateDate_YY_MM_DD(_form_value.docsdt)  :"";
        _form_value.docno = (_form_value && _form_value.docno) ? _form_value.docno :"";
        _form_value.docedt =  (_form_value && _form_value.docedt) ? FromateDate_YY_MM_DD(_form_value.docedt)  :"";
        _form_value.verifiedsdt = (_form_value && _form_value.verifiedsdt) ? FromateDate_YY_MM_DD(_form_value.verifiedsdt)  :"";
        _form_value.verifiededt =  (_form_value && _form_value.verifiededt) ? FromateDate_YY_MM_DD(_form_value.verifiededt)  :"";
    ...
    handleDate = (name, date) =>{
       if(name=="start_date"){
        ...
        else if(name=="verified_start_data"){
            this.setState({
                verified_start_data:date
            })
        }
        else if(name=="verified_end_data"){
            this.setState({
                verified_end_data:date
            })
        }
    ...
    ClearAll = () =>{
        this.props.reset('e2papproval_reject')
        this.setState({
            docstatus :'',
            all_check_value : [],
            verified_end_data : '',
            ...
            />
              ...
              <form onSubmit={handleSubmit(this.handlefromsubmit.bind(this))}>
                <div className="row mt-2">    
                    ...
                    {/* <Field type="text" name="Filters.strItemCode" component={FromInputsParallelSingle} className="form-control" placeholder="Item Code " label="Item Code" />        */}
                    <Field type="text" name="Filters.docsdt" selected={this.state.start_data} component={FormDatePickerParallelSingle} className="form-control" placeholder="Document Start Date " label="Document Start Date : "    onChange={this.handleDate.bind(this, 'start_date')} />
                    <Field type="text" name="Filters.docedt" selected={this.state.end_date}  component={FormDatePickerParallelSingle} className="form-control" placeholder="Document End Date " label="Document End Date : " minDate={this.state.start_data}  onChange={this.handleDate.bind(this, 'end_date')} /> 
                    {/* <Field type="text" name="Filters.strItemCode" selected={this.state.psd_start_data}  component={FormDatePickerParallelSingle} className="form-control" placeholder="PSD Sent Start Date " label="PSD Sent Start Date"  onChange={this.handleDate.bind(this, 'psd_start_data')} />       
                    <Field type="text" name="Filters.UIEndDate" selected={this.state.psd_end_date} component={FormDatePickerParallelSingle} className="form-control" placeholder="PSD Sent End Date " label="PSD Sent End Date" minDate={this.state.psd_start_data}  onChange={this.handleDate.bind(this, 'psd_end_data')} />  */}
                    <Field type="text" name="Filters.verifiedsdt" selected={this.state.verified_start_data}  component={FormDatePickerParallelSingle} className="form-control" placeholder="Payment Start Date " label="Verified Start Date : "  onChange={this.handleDate.bind(this, 'verified_start_data')} />       
                    <Field type="text" name="Filters.verifiededt" selected={this.state.verified_end_data} component={FormDatePickerParallelSingle} className="form-control" placeholder="Payment End Date " label="Verified End Date : " minDate={this.state.psd_end_data}  onChange={this.handleDate.bind(this, 'verified_end_data')} /> 
                    ...
                    {/* <Field type="text" name="Filters.Vendor" component={FromInputsParallelSingle} className="form-control" placeholder="Vendor" label="Vendor" />
                    <Field type="text" name="Filters.UIStartDate" component={FromInputsParallelSingle}  className="form-control" placeholder="  Vendor Address (1st Line) " label="  Vendor Address (1st Line)"   />
               */}
                    ...
                    <div className='col-12'>   
                        ...
                        <div className="row ">
                            <div className="col-lg-6 col-md-6 col-sm-12 col-12 main_details">
                                <Field type="text" name="Filters.status[18]"  component={FormRadioButtonSpan} onChange={this.getChecked} label="E2P Verified" checked={this.state.all_check_value.includes(18)}/> 
                                <Field type="text" name="Filters.status[17]"  component={FormRadioButtonSpan} onChange={this.getChecked} label="Department Approved" checked={this.state.all_check_value.includes(17)} /> 
                                <Field type="text" name="Filters.status[11]" component={FormRadioButtonSpan}  onChange={this.getChecked} label="Finance Verified"  checked={this.state.all_check_value.includes(11)}/> 
                                <Field type="text" name="Filters.status[12]"  component={FormRadioButtonSpan} onChange={this.getChecked} label="Finance Approved" checked={this.state.all_check_value.includes(12)}/> 
                                <Field type="text" name="Filters.status[13]"  component={FormRadioButtonSpan} onChange={this.getChecked} label="FM Approved" checked={this.state.all_check_value.includes(13)}/> 
                                <Field type="text" name="Filters.status[4]"  component={FormRadioButtonSpan} onChange={this.getChecked} label="Paid" checked={this.state.all_check_value.includes(4)}/> 
                                <Field type="text" name="Filters.status[15]"  component={FormRadioButtonSpan} onChange={this.getChecked} label="Void" checked={this.state.all_check_value.includes(15)}/> 
                                <Field type="text" name="Filters.status[14]"  component={FormRadioButtonSpan} onChange={this.getChecked} label="Rejected" checked={this.state.all_check_value.includes(14)}/> 
                            </div>
                        </div>
                    ...
                    <div className="col-12">
                        <div className="text-right mt-2 row">
                            <div className="col-12">
                                ...
                                {/* <button type="button" className="ml-4 btn btn-sm btn-outline-success">View Audit</button> */}
...
const mapDispatchToProps = dispatch => ({
    GetE2PApprovalAprovRej  : (values) => dispatch(GetE2PApprovalAprovRej(values)),
