...
class InvoiceViewDetails extends Component {
   ...
   async getSelectedProduct(e, details, selected){
        ...
        if(current_id!=null && current_model && table_body[current_id]){
    ...
    render(){
        ...
        let _table_header_1 = [
            ...
            {name : "Approving Officer", id:"AO_NAME", width:'100px',  formatter: (cellContent, row) => {
                console.log('updated_details',(row.FA_SEQ-1) == row.FA_AO_ACTION, row.FA_SEQ-1, row.FA_AO_ACTION)
                let _details =  (_selected_index==row.FA_SEQ || ((row.FA_SEQ-1) == row.FA_AO_ACTION)) ? 'bold_row' : ''
        ...
        return <div id="tabs">
                    <Tabs defaultActiveKey="VerifiedInvoice" transition={false} id="tabs" activeKey={this.state.active_key} onSelect={k => {
