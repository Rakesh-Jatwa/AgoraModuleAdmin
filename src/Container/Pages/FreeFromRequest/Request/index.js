...
class Request extends Component{
            ...
            _details_main.table_body = await _details_main.table_body.map((list_details, sub_index)=>{
                if(nextProps.delivery_address.length){
                    ...
                    if(_default_address.length){
                        ...
                        list_details.MPQ  = 1
                        list_details.GSTRATE = 'N/A'
                        list_details.change_gst = true 
                        list_details.Segmentation  = 'Standard Material'
                        list_details.Test  = '10'
                        list_details.sst_code_ls= list_details.taxCodeValues;
            ...
            _details_main.table_body = await _details_main.table_body.map((list_details, index)=>{
                ...
                if(nextProps.delivery_address.length){
                    ...
                    if(_default_address.length){
                        ...
                        _tem_details.GSTRATE =(_tem_details.GSTRATE) ? _tem_details.GSTRATE :  'N/A'
    ...
    componentDidUpdate(){
        console.log('componentDidUpdate', this.state.table_body)
        ...
        if(this.state.rerenderd && this.props.cost_centre_code && this.props.cost_centre_code.length){
            ...
            if(localStorage.getItem('free_from') && this.props.po_details && this.props.po_details.allInfo){
                  ...
                  let _user_details = {
                      ...
                      PaymentTerm : (_details.POM_PAYMENT_TERM) ? _details.POM_PAYMENT_TERM : '30 Days',
            ...
            if(this.state.table_body && this.state.table_body.length){
                 ...
                 this.state.table_body.forEach(async (item_details, index)=>{
                     ...
                     if(index >= _temp_values && item_details.removed !== 1){
                         ...
                         let _unit_cost = (item_details.PRODUCTCODE) ? 'itemList.'+index+'.'+item_details.PRODUCTCODE+'.UNITCOST' :  'itemList.'+index+'.'+index+'.UNITCOST';
                         let _custom_master_details = _custom_field_details.filter((list_cms,index_details)=>{
                            return  list_cms.PCD_PR_LINE == (index+1)
                         })
                         if(_custom_master_details.length>0){
                            _custom_master_details.forEach((list_cust,index)=>{
                                let _temp_details = _custom_field.filter((list,index)=>list.PCM_FIELD_NO == list_cust.PCD_FIELD_NO)
                                if(_temp_details.length){
                                     this.props.change((item_details.PRODUCTCODE) ? 'itemList.'+index+'.'+item_details.PRODUCTCODE+'.'+_temp_details[0].PCM_FIELD_NAME :  'itemList.'+index+'.'+index+'.'+_temp_details[0].PCM_FIELD_NAME , list_cust.PCD_FIELD_VALUE);
                                }
                            })
                         }
                         ...
                         await this.props.change(_unit_cost, (item_details.UNITCOST) ? item_details.UNITCOST : '0.00');
    ...
    async SelecthandleChange  (name, e){
        ...
        console.log('table_body_sel', _table_body, name, _value)
        if(_details && _value){
            let _details_name =  _details.split('.')
    ...
    async handlefiltersubmit_1(values){
        ...
        console.log('handlefiltersubmit_1',values)
       ...
       let _values = _table_body.forEach((list,index)=>{
        ...
        if(_validate_data){
            if(_table_body.length>0){
                ...
                console.log('_main_form_data', _main_form_data)
                ...
                _details.pr_cost = parseFloat(this.state.table_body.reduce((a, val) => a += (val.QUANTITY * val.UNITCOST) + val.GST, 0));
                ...
                console.log('_details.ShipVia', _details.ShipVia)
                        ...
                        // list_sub_item.forEach((list, index)=>{
                            ...
                            for (var i=0;i<_table_body.length; i++) {
                                if (_table_body[i].PRODUCTCODE == index && _table_body[i].PRODUCTCODE == index) {
                                    ...
                                    _table_body[i].PCD_PR_LINE = i+1;
                                    _table_body[i].MPQ = 1;
                                    _table_body[i].ITEMLINE = i+1;
                                    _table_body[i].ITEMINDEX = i+1;
                ...
                console.log('_customFieldDetailsMstr', JSON.stringify(_custom_field))
                ...
                if(_status.status){
                    if(_status.status && _submit_details=="submit"){
                        if(!_status.response.isBudgetExceed){
                            ...
                            this.state.table_body.forEach((list_updates)=>{
                                console.log('list_updates', list_updates.QUANTITY)
                            })
                            // console.log('po_approval_setup_ffpo', this.state.table_body.reduce((a, val) => a += (((val.QUANTITY) ? parseFloat(val.QUANTITY) : 0) * ((val.UNITCOST) ? parseFloat(val.UNITCOST) : 0)) + parseFloat(this.state.shipping_charge) , 0))
                            this.props.history.push({
                                pathname:'/po_approval_setup_ffpo',
                                datas: {
                                    'approval_list': _status.response.ApprovalSetup,
                                    'pr_no':  _status.message,
                                    'pr_cost': this.state.table_body.reduce((a, val) => a += (((val.QUANTITY) ? parseFloat(val.QUANTITY) : 0) * ((val.UNITCOST) ? parseFloat(val.UNITCOST) : 0))  , 0) + + parseFloat(this.state.shipping_charge)
                                }
                            })
                        }
                        else{
   ...
   download_file(details){
    ...
    commonfunction = async (raisePOForm) => {
        ...
        let billingAddress = {
            "pr_cost": this.state.purchaseRequestDetailsModifiedData.reduce((a, val) => a += parseFloat(val.QUANTITY * val.UNITCOST) + val.GST, 0),
                ...
                if(_status.status && _submit_details=="submit"){
                    this.props.history.push({
                        ...
                        datas: {
                            'approval_list': _status.response.ApprovalSetup,
                            'pr_no':  _status.message,
                            'pr_cost': this.state.table_body.reduce((a, val) => a += parseFloat((val.QUANTITY * val.UNITCOST) + val.GST, 0)),
                        }
                    })
                }
                else if(_status.status && _submit_details=="save"){
     ...
     saveItem_1 = async (_submit_details) =>{
        ...
        if(table_body.length){
            await table_body.forEach((list_details)=>{   
                ...
                else if((!list_details.hasOwnProperty('PERSONCODEDESC') || !list_details.PERSONCODEDESC) && (_user_details.UM_COY_ID=="pamb")){
                    this.setState({
                        validation:true,
                        modal_title : 'Validation',
    ...
    handleVendorChange = async(target) =>{
        if(target.value){
            ...
            if(_status){
                ...
                if(VendorDetail){
                    if(VendorDetail[0].paymentMethodDesc){
                        this.props.change('raisePOForm.PaymentType', VendorDetail[0].paymentMethodDesc)
                        this.props.change('raisePOForm.PaymentTerm', VendorDetail[0].paymentTermDesc)
                    }
                   
                    this.setState({
                        billing_method : (VendorDetail[0].CV_BILLING_METHOD) ? VendorDetail[0].CV_BILLING_METHOD : 'GRN',
                        loading :false
                    })
                }
                else{
