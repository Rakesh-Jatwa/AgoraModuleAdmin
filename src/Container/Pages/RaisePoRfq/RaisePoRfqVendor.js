...
class Request extends Component{
    ...
    static async getDerivedStateFromProps(nextProps, prevState){
        ...
        if((!prevState.rendered) && nextProps.location && nextProps.location.datas){
            ...
            _details_main.internal_attachment = (nextProps.location && nextProps.location.datas) ? nextProps.location.datas.internalAttach : [];
            _details_main.extrnal_attachment = (nextProps.location && nextProps.location.datas) ? nextProps.location.datas.externalAttach : [];
            ...
            _details_main.customFieldDetailsMstr =(nextProps.location && nextProps.location.datas && nextProps.location.datas.dsItem ) ? nextProps.location.datas.dsItem.customFieldDetails : 0;
            ...
            _details_main.table_body = await _details_main.table_body.forEach(async (list_details, index)=>{
                ...
                _details_main.table_body[index].change_gst =  (list_details.GST!='N/A' && list_details.GST!='n/a' && list_details.GST!='') ?  true  : false;
                // _details_main.table_body[index].change_gst = true;
                ...
                _details_main.table_body[index].sst_code_ls = (list_details.taxCodeValues) ?  list_details.taxCodeValues : [];
                _details_main.table_body[index].costCentre = (list_details.costCenter) ?  list_details.costCenter : '';
                _details_main.table_body[index].PRODUCTCODE = (list_details.PRODUCTCODE) ?  list_details.PRODUCTCODE : list_details.ITEMLINE;
                ...
                _details_main.table_body[index].default_dl_address = (list_details.delivaryAddressCode && list_details.delivaryAddressCode!=null) ?  list_details.delivaryAddressCode : _details_main.table_body[index].DeliveryAddr;
                ...
                if(_details_main.customFieldMstr && _details_main.customFieldMstr.length) {
                    _details_main.customFieldMstr.forEach((list_details)=>{
                        let _temp_details = _details_main.customFieldDetailsMstr.filter((list_details_main) => ((list_details_main.PCD_PR_LINE == _details_main.table_body[index].ITEMLINE) &&  (list_details_main.PCD_FIELD_NO == list_details.PCM_FIELD_NO)))
                        if(_temp_details && _temp_details.length){
                            _details_main.table_body[index][list_details.PCM_FIELD_NAME] = _temp_details[0].PCD_FIELD_VALUE
                        }
                    })
                } 
                await nextProps.location.datas.rfqItemDetails.forEach((list_details)=>{
                    if(nextProps.delivery_address.length){
                        let _default_address = nextProps.delivery_address.filter((list)=>{return list.AM_ADDR_CODE===_details_main.table_body[index].default_dl_address})
                        if(_default_address.length){
                            _details_main.table_body[index].deliveryAddress = _default_address[0].Address
                            _details_main.table_body[index].DeliveryAddr =  _details_main.table_body[index].DeliveryAddr
    ...
    async componentDidMount(){
        ...
        if(localStorage.getItem('po_from_rfq')){
            let _details = JSON.parse(localStorage.getItem('po_from_rfq'))
            this.setState({
    ...
    componentDidUpdate(){
        ...
        let {QuotationShipment} = this.state
        if(!this.state.rerenderd && this.props.cost_centre_code && this.props.cost_centre_code.length){
            ...
            // if(this.props.purchased_items && this.props.purchased_items.rfqItemDetails){
                  ...
                  let _table_body_fetched = this.state.table_body
                  if(let_details && (this.props.po_details && !this.props.po_details.hasOwnProperty('allInfo'))){
                     
                        console.log('_table_body_fetched',_table_body_fetched)
                        ...
                        _state_data.ExternalRemark = (RFQ_Mstr.remark) ? RFQ_Mstr.remark  : "";
                        _state_data.InternalRemark = (RFQ_Mstr.strPR_In_Remark) ? RFQ_Mstr.strPR_In_Remark  : "";
                        // _state_data.ShipmentTerm = (RFQ_Mstr.ship_term) ? RFQ_Mstr.ship_term : "Not Applicable";
                        _state_data.ShipmentTerm = (QuotationShipment) ? QuotationShipment.strShipTermDesc : "Not Applicable";
                        _state_data.ShipmentMode = (QuotationShipment) ? QuotationShipment.strShipModeDesc  : "Not Applicable";
                        ...
                        let _main_object = {}
                        if(_table_body_fetched.length){
                            let _temp_array = [];
                            let _item_details = Object.assign([], _table_body_fetched)
                            _item_details.forEach((item_details, index)=>{

                                let _cost_center = this.props.cost_centre_code.filter((list,index)=>{return index==0})
                                if(_cost_center.length){
                                    _table_body_fetched[index].ACCT = _cost_center[0].AM_ACCT_INDEX;
                                    _table_body_fetched[index].prdAcctIndex = _cost_center[0].AM_ACCT_INDEX;
                                    _table_body_fetched[index].costCentre = _cost_center[0].CDM_DEPT_CODE+'-'+_cost_center[0].AM_ACCT_DESC+':'+_cost_center[0].AM_ACCT_CODE;
                                }
                                ...
                                let _build_array = new Object()
                                  _build_array = {
                                      [delivery] : { 
                                          ...
                                          "delivery" :addDate(new Date(), 1),
                                          "POD_ORDERED_QTY":item_details.QUANTITY,
                            ...
                            console.log('_item_details',_temp_array)
                            _main_object = {
                                raisePOForm : _state_data,
                        ...
                        this.setState({
                                rerenderd:true,
                                Urgent:(_state_data.Urgent) ? 1 : 0,
                                strPrNO : '',
                                table_body:_table_body_fetched,
                                POData : _state_data,
                                address :  {BillAddrCode : this.state.BillAddrCode},
                                billing_method : let_details.CV_BILLING_METHOD
                        })
                  }
                    ...
                    _item_list.forEach((list,index)=>{
                          ...
                          temp_data.MPQ = 1
                          temp_data.ITEMINDEX = list.ITEMINDEX;
                          temp_data.ITEMLINE = list.ITEMLINE;
    ...
    async handlefiltersubmit(values, details){
        ...
        if(let_details.billingAddress){
            ...
            _POData.Urgent = (this.state.Urgent==1 || this.state.Urgent==true) ? 1 : 0
            ...
            _table_body.forEach((list,index)=>{
                ...
                _table_body[index].POD_RFQ_ITEM_LINE = _table_body.ITEMLINE
                _table_body[index].MPQ = 1;
                if(this.props.gl_code && this.props.gl_code.dvwCus){
                    this.props.gl_code.dvwCus.forEach((list_details)=>{
                        if((_table_body[index] && _table_body[index]!==undefined) && _table_body[index].hasOwnProperty(list_details.CF_FIELD_NAME) ){
                            console.log('list_details.CF_FIELD_NAME', list_details.CF_FIELD_NAME, _table_body[index][list_details.CF_FIELD_NAME])
                            _custom_field.push({
                                "PCD_PR_INDEX": _item_main_index,
                                "PCD_PR_LINE" : _item_main_index,
                                "PCD_FIELD_NO": list_details.CF_FIELD_NO,
                                "PCD_FIELD_VALUE": _table_body[index][list_details.CF_FIELD_NAME]
                            })
                        }
            ...
            if(_save_item){
                let _final_datas ={
                        "POCost":this.state.table_body.reduce((a, val) =>a += (val.QUANTITY * val.UNITCOST) + ((val.GST/100) * (val.QUANTITY * val.UNITCOST)), 0).toFixed(2),
                        "hidBaseAmt":1,
                        ...
                        customFieldDetailsMstr : _custom_field,
                }

                console.log('_final_datas', _final_datas)
                ...
                console.log('_temp_details', _final_datas)
                ...
                if(_status  && values.submit_type=="submit"){
                    ...
                    if(_status.status){
                        if(_status.response && _status.response.po){
                            ...
                            localStorage.setItem('po_from_rfq', JSON.stringify({POM_PO_NO:_temo_po_det.poNo,POM_PO_INDEX:_temo_po_det.poIndex}))
                        }
                        ...
                        if(_status.status && _rfq && (!_status.response.isBudgetExceed)){
                            ...
                            this.props.history.push({
                                pathname:'/po_approval_setup_rfq',
                                datas: temp,
                                redirect_to_page : 'raise_po_rfq_vendor'
                            })
                        }
                        else{
                            if(_status.response && _status.response.po){
            ...
            this.setState({
                modal_title: 'n',
                title : '',
                modal_body: 'Update Default billing Address',
                status : false,
                validation: true,
            })
        }
   ...
   download_file(details){
        ...
        if(_details && _value){
            ...
            if(_qty_input=='GSTRate'){
                if(_table_body[name].change_gst){
                    ...
                    this.setState({ loading:true})
                    let _status =  await ApiExtract(FillTaxCode, {strRate:value.options[value.selectedIndex].text})
                    if(_status){
                        this.setState({
                            loading:false
                        })
    ...
    let _sub_total = this.state.table_body.reduce((a, val) => a += val.QUANTITY * val.UNITCOST, 0).toFixed(2)
    let _gst_total = this.state.table_body.reduce((a, val) => a += ((val.GST/100)* (val.QUANTITY * val.UNITCOST)), 0).toFixed(2)
    let _shipping_amt = (this.state && this.state.shipping_charge) ? this.state.shipping_charge : 0;
    let _grand_total = parseFloat(_sub_total) + parseFloat(_gst_total) + parseFloat(_shipping_amt)
     const { handleSubmit } = this.props
        const _table_header = [
            ...
            {name : "PO Qty", id:"POD_ORDERED_QTY", key:false, disabled:true, width:'67px',type:"number"},
            {name : "UOM", id:"UOM",width:'100px'},
            {name : "Unit Price", id:"UNITCOST",  disabled:true, width:'100px',type:"number"},
            ...
            {name : "SST Amount", id:"GST", key:false, width:'100px', type:"gst_rate", align:'right'},
            ...
            {name : "Delivery Address", id:"deliveryAddress", key:false, type:"click", width:'186px'},
            {name : "Est. Date of Delivery (dd/mm/yyyy)", id:"delivery", key:false, type:"date", width:'143px'},
            {name : "Warranty Terms (months)", id:"WARRANTYTERMS", key:false,  width:'107px',type:"number"},
              ...
              <TabHeading color={'bg-info color-white'}>Purchase Order Header</TabHeading> 
                ...
                <div className="row mt-2">    
                    <div className='col-12'>
                        <div className="row mt-2"> 
                            ...
                            <div className="col-12 col-md-6">
                                <div className="row mt-2">
                                    <Field type="text" name="raisePOForm.CurrencyCode" component={FromInputs} className="form-control" placeholder="Currency Code" label="Currency Code :"  readonly={true}/>
                                </div>
                            </div>
                            <div className="col-12 col-md-6">
                                <div className="row mt-2">
                                    <Field className="form-control textboxstyle" defaultValue={'30 Days'} rem={true} component={FromSelect} label="Payment Terms " name="raisePOForm.PaymentType">
                            ...
                            <div className="col-12 col-md-6">
                                <div className="row mt-2">
                                    <Field className="form-control textboxstyle" component={FromSelect} name="raisePOForm.PaymentMethod" label="Payment Method:"  rem={true}>
                            ...
                            <div className="col-12 col-md-6">
                                <div className="row mt-2">
                                    <Field className="form-control textboxstyle" component={FromSelect} name="raisePOForm.ShipmentTerm" label=" Shipment Terms :"  rem={true}>
                                        <option defaultValue=''>--Select--</option>
                            ...
                            <div className="col-12 col-md-6">
                                <div className="row mt-2">
                                    <Field className="form-control textboxstyle" component={FromSelect} name="raisePOForm.ShipmentMode" label=" Shipment Mode :" rem={true}>
                                        <option defaultValue=''>--Select--</option>
                ...
                <div className="row mt-2">    
                    <div className='col-12'>   
                        ...
                        <BootstrapCustomTable 
                            ...
                            subtotal={_sub_total}
                            sstamount={_gst_total}
                            grandtotal={_grand_total}
                            table_id="line"
                ...
                <div className="col-12 mt-3 text-center">
                    {this.state.show_save_button && <Fragment> 
                        ...
                        {/* <div className="display_inline"><BackButton back_data={(this.props.location  && this.props.location.redirect_to_page) ? true : false}  {...this.props.location} goBack={this.props.history} history={this.props.history}/></div> */}
                    <button className="btn btn-sm btn-outline-danger ml-2" onClick={()=>this.GoBack()}>Back</button>
...
const mapStateToProps = state => ({
    ...
    request_form : state.form.raise_po_main_rfq,
