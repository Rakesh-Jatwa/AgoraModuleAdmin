...
import {UploadDocuments, GetGenerateCREDITPDF} from '../../../../Actions/Requester'
...
import {GetCNListing, GetCNDetails, GetEmptyCNDN } from '../../../../Actions/Vendor'
import {ApiExtract} from '../../../../Common/GetDatas'
import {GetSubmitCN} from '../../../../Apis/Vendor'
...
class CreditView extends Component {
    ...
    componentDidMount(){
        this.props.GetCNDetails({invoiceNo : this.props.invoice_number})
    }

   
    static getDerivedStateFromProps(props, state){
        let _details = [];
         if((!state.file_upload) && (!state.delete) && props.cn_details && props.cn_details.hasOwnProperty('invDetails') && (!state.render)){
           return {
                invDetails : (props.cn_details && props.cn_details.invDetails) ? props.cn_details.invDetails[0] : [],
                lineItems : (props.cn_details && props.cn_details.lineItems) ? props.cn_details.lineItems : [],
                relatedCreditNotes : (props.cn_details && props.cn_details.relatedCreditNotes) ? props.cn_details.relatedCreditNotes : [],
                // render :  (this.state.cn_details && this.state.cn_details.lineItems  && this.state.cn_details.lineItems.length > 0) ? true : false,
            }
         }
    ...
    componentDidUpdate(){
        if(this.props.cn_details && this.props.cn_details.lineItems && this.props.cn_details.lineItems.length>0 && this.state.render_table_input==false){
            let _temp_details = this.props.cn_details.lineItems;
            if(_temp_details.length){
                let _temp_array = new Array();
                _temp_details.forEach((enumObject, index)=>{
                    console.log('componentDidUpdate', enumObject)
                   _temp_array[index] = {
                        qty : '0.00',
                        unit_price : '0.00',
                        Amount_1 : '0.00',
                        gst_rate : enumObject.ID_GST_RATE,
                        output_tax_code : enumObject.ID_GST_RATE,
    ...
    SubmitDetails = async() =>{
        ...
        let _sub_total  = table_inputs ? table_inputs.reduce((a, val) => a += val.qty * val.unit_price, 0) : '0.00';
        let _sub_gst  = table_inputs ? table_inputs.reduce((a, val) => a  += (val.ID_GST) ? (val.qty * val.unit_price) * (val.ID_GST/100) : 0 , 0) :  '0.00';
        _sub_total = parseFloat(_sub_total).toFixed(2);
        _sub_gst = parseFloat(_sub_gst).toFixed(2);
        ...
        if(_temp_submit_details){
            let _details ={
                ...
                "strInvNo":   (this.props.invoice_number) ? this.props.invoice_number : '',
                "decTotal":_total,
            ...
            let _status = await ApiExtract(GetSubmitCN, _details);
            if(_status){
                this.setState({
                    model:false,
                    loading:false,
    ...
    handleTableInputs(details, names, new_details){
        ...
        console.log('_new_details', _new_details[`${new_details}`] )
        // _new_details[`${new_details}`]  = 
    ...
    saveItem = async (_submit_details) =>{
        let {table_inputs} = this.state;
        console.log('saveItem', table_inputs)
        let _date_inst = new Date();
        let _result = true;
        let _prev_status = false;
        if(table_inputs.length){
            await table_inputs.some((list_details, index_details)=>{  
                ...
                else if(!list_details.hasOwnProperty('ID_RECEIVED_QTY') || (parseFloat(list_details.qty) > list_details.ID_RECEIVED_QTY)){
                    this.setState({
                        show:true,
                        modal_title : '',
                        status :false,
                        modal_body : `Line ${index_details}. Credit Note Item Qty has exceed invoice item Qty`
                    })
                    _result = false
                    return true
                } 
    ...
    ViewPdf = () =>{
        ...
        let _temp_details = {"CN_NO":(this.state.do_number) ? this.state.do_number:'', "CN_SCoyID":(lineItems && lineItems.length>0) ? lineItems[0].ID_S_COY_ID : '', "CN_BCoyID":(lineItems && lineItems.length>0) ? lineItems[0].IM_B_COY_ID : ''}
    ...
    render(){
        let _table_data_header = [
            ...
            {name : "SST Rate", id:"ID_GST_RATE", width:'85px',dataFormat:"text"},
            ...
            {name : "Remarks", id:"remarks", width:'144px', dataFormat:"cn_textarea"},
        ];

        let _sub_total  = this.state.table_inputs  ? this.state.table_inputs.reduce((a, val) => a += val.qty * val.unit_price, 0) : '0.00';
        let _sub_gst  = this.state.table_inputs  ? this.state.table_inputs.reduce((a, val) => a  += (val.ID_GST && val.ID_GST_RATE!="N/A") ? (val.qty * val.unit_price) * (val.ID_GST/100) : 0 , 0) :  '0.00';
        _sub_total = parseFloat(_sub_total).toFixed(2);
        _sub_gst = parseFloat(_sub_gst).toFixed(2);
        let _total  = (parseFloat(_sub_total)+parseFloat(_sub_gst)).toFixed(2)
                        ...
                        <div className="col-md-12">
                            ...
                            <div className="row" >
                                ...
                                <div className="col-12 col-sm-12 col-md-6">
                                    ...
                                    <div className="row" >
                                        <div className="col-md-12 col-12 col-lg-4"><label>Related Credit Note : </label></div>
                                        <div className="col">{(this.state.relatedCreditNotes && this.state.relatedCreditNotes.length > 0) ? this.state.relatedCreditNotes.map((elem)=> <div onClick={()=>  this.props.GetGenerateCREDITPDF({"CN_NO":elem.CNM_CN_NO, "CN_SCoyID":elem.CNM_CN_S_COY_ID, "CN_BCoyID":elem.CNM_CN_B_COY_ID})} className="invoice_inline">{elem.CNM_CN_NO},</div>):''}</div>
                                    </div>
                                    <div className="row">
                                        <div className="col-md-12 col-12 col-lg-4"><label>Total Related Credit Note Amount: </label></div>
                                        
                                        <div className="col"><p>{(this.state.relatedCreditNotes && this.state.relatedCreditNotes.length > 0) ?  this.state.relatedCreditNotes.reduce((a, val) => a += (val.CNM_CN_TOTAL), 0).toFixed(2) :''}</p></div>
                                    </div>
                                    <div className="row">
...
const mapStateToProps = state => ({
    cn_details : state.cn_details.responseList,
    cn_loading : state.cn_details.loading,
  ...
  const mapDispatchToProps = dispatch => ({
    GetCNDetails: (values) => dispatch(GetCNDetails(values)),
...
const CreditViewMain = connect(mapStateToProps, mapDispatchToProps)(CreditView);
let CreditViewHolder = reduxForm({
    form: 'ConverPRListing'
})(CreditViewMain)
export default CreditViewHolder