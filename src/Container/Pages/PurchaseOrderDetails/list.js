...
import {DuplicatePO} from '../../../Apis/Approver'
...
import {get_po_status_int, psa_po_status, getstrStatus, HandlePaymentTerm, getPOStatusLsiting, FileNotFound} from '../../../Actions/Common/Functions'
...
class PurchaseOrderDetails extends Component {
    ...
    async DuplicatePo(){
        ...
        if(this.props.purchase_reqest_approval  &&  this.props.purchase_reqest_approval.get_PODetail && this.props.purchase_reqest_approval.get_PODetail && this.props.purchase_reqest_approval.get_PODetail.poDEtails.length>0){
            this.setState({loading:true})
            let _status = await ApiExtract(DuplicatePO, {
                "strPONo":this.props.purchase_reqest_approval.get_PODetail.poDEtails[0].POM_PO_NO,
                "intPOIndex":this.props.purchase_reqest_approval.get_PODetail.poDEtails[0].POM_PO_INDEX,
            });
            if(_status){
                this.setState({
                    status: _status.status,
    ...
    componentDidUpdate(){
        if(this.props.purchase_reqest_approval && this.props.purchase_reqest_approval.getlineitem && this.props.purchase_reqest_approval.getlineitem.lineitemDetails && this.props.purchase_reqest_approval.getlineitem.lineitemDetails.length){
            ...
            if(this.props.purchase_reqest_approval &&  this.props.purchase_reqest_approval.get_PODetail && this.props.purchase_reqest_approval.get_PODetail && this.props.purchase_reqest_approval.get_PODetail.poDEtails.length && (!this.state.updated_status)){
                let _temp_Details = this.props.purchase_reqest_approval.get_PODetail.poDEtails[0]
                if(_temp_Details.STATUS_DESC=="Closed" || _temp_Details.STATUS_DESC=="Pending Approval" || _temp_Details.STATUS_DESC=="Void" || _temp_Details.STATUS_DESC=="Submitted" || _temp_Details.STATUS_DESC=="submitted"){
                    this.setState({ 
                        show_duplicate :true,
                        updated_status : true
                    });
    ...
    render(){
        ...
        if(this.props && this.props.location && this.props.location.page_name=='ConvertPRListing'){
            ...
            _table_header = [
                ...
                {name : "UOM", id:"POD_UOM", width:'100px'},
                {name : "Est. Date of Delivery", id:"POD_D_ADDR_CODE", width:'100px',formatter: (cellContent, row) => {
                    if(this.props.purchase_reqest_approval &&  this.props.purchase_reqest_approval.get_PODetail && this.props.purchase_reqest_approval.get_PODetail && this.props.purchase_reqest_approval.get_PODetail.poDEtails.length){
                        return addDate(this.props.purchase_reqest_approval.get_PODetail.poDEtails[0].POM_CREATED_DATE, row.POD_ETD)
                    }
                    else{
                        return addDate(this.state.products.POM_SUBMIT_DATE, row.POD_ETD)
                    }
                ...
                <TabHeading color={'bg-info text-white'}>Purchase Order Header</TabHeading> 
                    <div  className="row mt-2">
                        ...
                        {((this.props.location && this.props.location.redirect_to_tab && this.props.location.redirect_to_tab=="POListing") ) ?<Fragment>
                            ...
                            <div className="col-12 col-md-2 col-lg-2"><label>Status :  </label></div>
                            <div className="col"><p>{(this.state.products) ? psa_po_status(this.state.products) :''} </p></div>
                        </Fragment>  :<Fragment>
                            <div className="col-12 col-md-2 col-lg-2"><label>Status :  </label></div>
                   ...
                   {(this.state.products && this.state.products.page_name &&  this.state.products.page_name=="vendor_po_listing") ?
                    ...
                    <div className='col-12' className="mb-2 mt-2"><a  onClick={FileNotFound}  target="_blank" style={{textDecoration:'underline', color:'blue'}}>Click here</a> to download the Term & Conditions document</div> 
                                ...
                                <table className="table table-hover table-stripe">
                                    ...
                                    <tbody>
                                        {this.props.purchase_reqest_approval.approvalFlow.approvalFlowDetails.map((data, index) =>
                                            <tr key={index} className={((_user_details.UM_USER_NAME==data.AO_NAME  || _user_details.UM_USER_NAME==data.AAO_NAME) || ((data.PRA_SEQ-1) == data.PRA_AO_ACTION)) ? 'bold_row' : ''}>
                                                <td>{data.PRA_SEQ}</td>
                                                <td>{data.AO_NAME}</td>
                        ...
                        <div className="mt-2 mb-5 justify-content-end row">
                            {(this.state.show_duplicate) ? <div className="col-lg-auto col-md"><button type="button" className="ml-4 btn btn-sm btn-outline-primary" onClick={()=>this.confirm_function('duplicate', 'duplicate This PO')} >Duplicate PO</button></div> : '' }
                            <div className="col-lg-auto col-md"><button type="button" className="btn btn-sm btn-outline-success" onClick={()=>this.GetGeneratePOPDF()}>View PO</button></div>
